...
```{r}
processor <- COVID19DataProcessor$new(provider = "JohnsHopkingsUniversity", missing.values = "imputation")
processor$setupStrategies()

dummy <- processor$setupData()
dummy <- processor$curate()

current.date <- max(processor$data$date)
